...
def _get_launch_data():
    ...
    tmp = Path("/home/chirag/airflow/appfiles/tmp/launches.json")
    data = {}
    if not tmp.is_file():
        try:
            ...
            print(e)
        Path("/home/chirag/airflow/appfiles/tmp").mkdir(parents=True, exist_ok=True)
        with open(tmp, "w") as json_file:
            json.dump(data, json_file, indent=4)


def _get_pictures():
    tmp = Path("/home/chirag/airflow/appfiles/tmp/images")
    if not tmp.is_dir():
        tmp.mkdir(parents=True, exist_ok=True)

        # Download all pictures in the launches.json
        with open("/home/chirag/airflow/appfiles/tmp/launches.json") as f:
